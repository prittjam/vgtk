...
%
function F = est_F_from_7p_mle(u,F0,varargin)
...
function F = go_bartoli(u,F0,W)
    ...
    err_fn = @(x) reshape(bsxfun(@times,sqrt(W'), EG.sampson_F_dist(u,unwrap(x))),[],1);
...
function F = go_overparam(u,F0,W,R)
    ...
    err_fn = @(x) ...
             reshape(bsxfun(@times,sqrt(W'),R'\ EG.sampson_F_dist(u,unwrap(x))),[],1);
...
function err = overparam_err_fn(x,u,W,R)
    ...
    err0 = reshape(bsxfun(@times,sqrt(W'),...
                          R'\ EG.sampson_F_dist(u,unwrap(x))),[],1);
    err = tukey(err0);
